...
with open('nutrients.pkl', 'rb') as f:
    b = pickle.load(f)
with open('servings.pkl', 'rb') as f:
...
def similarity_object(recipe_id):
    ...
    sims = gensim.similarities.Similarity.load('similarity_recipe_name', mmap=None)
...
def recommend(recipe_id,string):
    sims,dictionary,tf_idf = similarity_object(recipe_id)
    query_doc = [w.lower() for w in tokeniser(string)]
    query_doc_bow = dictionary.doc2bow(query_doc)
    query_doc_tf_idf = tf_idf[query_doc_bow]
    rec_recipe = [recipe_id[x] for x in sims[query_doc_tf_idf].argsort()[-5:][::-1] if len(b[x]) !=0]
    new_ing  = [ingredients[x] for x in sims[query_doc_tf_idf].argsort()[-5:][::-1]if len(b[x]) !=0]
    new_nutrition = [b[x] for x in sims[query_doc_tf_idf].argsort()[-5:][::-1] if len(b[x])!=0]
    #print(new_ing)
    new_servings = [servings[x] for x in sims[query_doc_tf_idf].argsort()[-5:][::-1] if len(b[x])!=0]
    
    z = sorted(range(len([b[x] for x in sims[query_doc_tf_idf].argsort()[-5:][::-1] if len(b[x]) !=0])),
           key=lambda x:[b[y] for y in sims[query_doc_tf_idf].argsort()[-5:][::-1] if len(b[y]) !=0][x][0][2])
...
recommend(recipe_id,' '.join(sys.argv[1:]))
