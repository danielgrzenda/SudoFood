...
from application.forms import LoginForm, RegistrationForm, EditProfileForm, EnterRecipeForm
...
def enter_recipe():
    ...
    if form.validate_on_submit():
        recipe = InputRecipe(title=form.title.data, servings=form.servings.data, ingredients=form.ingredients.data, user_id=current_user.id)    
        db.session.add(recipe)
        db.session.commit()
        flash('Your recipe was added')
        return redirect(url_for('enter_recipe'))
    return render_template("enter_recipe.html", title="Enter Recipe", form=form)
